...
search_url = (
    ...
    f"?keywords={quote(keyword)}&location={quote(location)}&geoId={geoId}&f&f_TPR={time_filter}&start={{}}"
)
...
while True:
    ...
    if resp.status_code != 200:
        print(
            f"Failed to retrieve jobs page (status code {resp.status_code}). Stopping."
        )
        break
    ...
    for card in tqdm(job_cards, desc=f"processing page start {start}", leave=False):
        ...
        if job_link and job_link.startswith("/"):
            job_link = (
                "https://www.linkedin.com" + job_link
            )  # convert relative link to absolute

        # Posted date (inside <time> tag). It might have 'datetime' attribute or just text.
        date_elem = card.find("time", class_="job-search-card__listdate") or card.find(
        ...
        if job_id:
            ...
            if detail_resp.status_code == 200:
                ...
                desc_container = detail_soup.find(
                    lambda tag: tag.name == "div"
                    and tag.get_text()
                    and "description" in (tag.get("class") or [])
                )
                # Fallback: find any element with 'description' in class name
                if not desc_container:
                    desc_container = detail_soup.find(
                        "div", {"class": lambda x: x and "description" in x}
                    )
                if desc_container:
...
try:
    with open(output_file, mode="w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f, fieldnames=["Job Title", "Company", "Job Link", "Posting Date"]
        )
        writer.writeheader()
